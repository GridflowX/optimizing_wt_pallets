...
def generate_bimodal_bus_arrival_time():
    """
    Generates a passenger arrival time using a bimodal Gaussian distribution.
...
def save_to_csv(total_passenger_count, capacity):
    filename = f"exp2_passengerdata_{capacity}_{total_passenger_count}s.csv"  # Changed to exp2
    try:
...
# Modified function to log cost value to SQLite tables and calculate avg_waiting_time
...
def main_simulation(total_passenger_count, capacity):
    ...
    np.random.seed(total_passenger_count)  # Seed numpy's random number generator too
    ...
    for i in range(total_passenger_count):
        ...
        arrival_time = generate_bimodal_bus_arrival_time()  # Use new bimodal function
    ...
    # Debug: Verify initial passenger state
    ...
    while True:
        ...
        if remaining_passengers == 0:
            ...
            print(f"Debug: total_passenger_count before save_to_csv = {total_passenger_count}")
            save_to_csv(total_passenger_count, capacity)
            print(f"Debug: total_passenger_count before results = {total_passenger_count}")
...
passenger_counts = [896]  # Updated passenger counts
capacities = [50, 80]
