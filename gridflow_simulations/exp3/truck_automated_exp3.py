...
def save_to_csv(total_passenger_count, capacity):
    filename = f"exp3_passengerdata_{capacity}_{total_passenger_count}.csv"
    try:
...
def main_simulation(total_passenger_count, capacity):
    ...
    outskirts_stops = ['A', 'C', 'D', 'P', 'Q', 'R', 'S']
    central_stop = 'B'
    b_destined_passengers = int(total_passenger_count * 0.7)
    other_destined_passengers = total_passenger_count - b_destined_passengers
    for i in range(b_destined_passengers):
    ...
    for i in range(other_destined_passengers):
        while True:
            arrival_stop = random.choice(outskirts_stops)
    ...
    conn.commit()
    print(f"{total_passenger_count} Passengers added successfully for capacity {capacity} (70% destined to 'B', 30% to others).")
...
for passenger_count in passenger_counts:
    for capacity in capacities:
        print(f"\nStarting exp3 simulation for {passenger_count} passengers with capacity {capacity}...")
